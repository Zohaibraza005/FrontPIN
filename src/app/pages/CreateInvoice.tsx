...
import { invoiceAPI, projectAPI, invoiceCompanyAPI } from '../services/api'; // adjust imports
...
export default function CreateInvoice() {
  ...
  const [items, setItems] = useState<InvoiceItem[]>([
    { name: '', description: '', quantity: 1, unitPrice: 0, discountType: 'flat', discountValue: 0 },
  ]);
  ...
  // Create Invoice
  const handleCreateInvoice = async (status: "UNPAID" | "DRAFT") => {
  ...
  <Button
    onClick={() => handleCreateInvoice("UNPAID")}
  >
    Create Invoice
  </Button>
</div>
