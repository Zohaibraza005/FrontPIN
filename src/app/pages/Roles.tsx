...
import { Search, Plus, Edit, Trash2, Shield, Users, RefreshCw, CheckCircle2, Lock, Sparkles, AlertTriangle } from "lucide-react";
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl p-6">
          <DialogHeader>
            ...
            <p className="text-xs text-gray-500">
              Configure the role name, description, and assign specific CRUD access levels.
            </p>
          </DialogHeader>
          ...
          <form onSubmit={handleSaveRole} className="space-y-5 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5 sm:col-span-1">
                ...
                <Input
                  placeholder="e.g. HR Manager, Accountant, Team Lead"
                  value={roleName}
                  onChange={(e) => setRoleName(e.target.value)}
                  className="h-10 rounded-xl border-gray-300 text-sm font-medium"
                  required
                />
              </div>
              ...
              <div className="space-y-1.5 sm:col-span-1">
                ...
                <Input
                  placeholder="Brief note about this role's scope"
                  value={roleDescription}
                  onChange={(e) => setRoleDescription(e.target.value)}
                  className="h-10 rounded-xl border-gray-300 text-sm"
                />
              </div>
            </div>

            {/* Privileges Table */}
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden space-y-3 p-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b pb-3">
                <div>
                  <h4 className="text-xs font-bold text-gray-900 flex items-center gap-1.5">
                    ...
                    Module Permissions & CRUD Access
                  </h4>
                  <p className="text-[11px] text-gray-500">
                    Control which actions users assigned to this role can execute.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="text-[11px] h-7 px-2.5 text-blue-600 border-blue-200 hover:bg-blue-50 rounded-lg"
                  >
                    Select All
                  </Button>
                  <Button
                    ...
                    className="text-[11px] h-7 px-2.5 text-indigo-600 border-indigo-200 hover:bg-indigo-50 rounded-lg"
                  >
                    Read Only
                  </Button>
                  <Button
                    ...
                    className="text-[11px] h-7 px-2.5 text-gray-600 border-gray-200 hover:bg-gray-50 rounded-lg"
                  >
                    Clear All
                  </Button>
                </div>
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden text-xs">
                <div className="grid grid-cols-6 bg-gray-50 px-3.5 py-2 font-bold text-gray-700 border-b border-gray-200">
                ...
                <div className="divide-y divide-gray-100 max-h-[320px] overflow-y-auto">
                  {privilegesList.map((mod, index) => (
                    <div
                      key={mod.id}
                      className={`grid grid-cols-6 items-center px-3.5 py-2.5 hover:bg-blue-50/30 transition-colors ${
                        index % 2 === 0 ? "bg-white" : "bg-gray-50/40"
                      }`}
                    >
                      <div className="col-span-1 pr-2">
                        <p className="font-semibold text-gray-900 text-xs">{mod.label}</p>
                        <p className="text-[10px] text-gray-400 hidden sm:block truncate">{mod.description}</p>
                      </div>

                      <div className="flex justify-center">
                        <Checkbox
                          checked={isChecked(mod.id, "create")}
                      ...
                      <div className="flex justify-center">
                        <Checkbox
                          checked={isChecked(mod.id, "update")}
                      ...
                      <div className="flex justify-center">
                        <Checkbox
                          checked={isChecked(mod.id, "delete")}
                      ...
                      <div className="flex justify-center">
                        {mod.id === "report" ? (
                          <Checkbox
                            checked={isChecked(mod.id, "ownTeamOnly")}
                            onCheckedChange={() => togglePermission(mod.id, "ownTeamOnly")}
                          />
                        ) : (
            ...
            <DialogFooter className="gap-2 sm:gap-0 pt-2 border-t">
              ...
              <Button
                type="submit"
                disabled={saving}
                className="rounded-xl h-10 bg-blue-600 hover:bg-blue-700 text-white font-medium px-5"
              >
                {saving ? (
                  <>
